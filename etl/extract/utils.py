...
def save_to_file(data, api_type):
    ...
    if os.path.exists(file_path):
        ...
        try:
            ...
            logger.warning(
                f"Could not decode JSON in {file_path}, starting with empty list"
            )
            existing_data = []
    ...
    if api_type == 'gold':
        ...
        logger.debug(
            f"Checking for duplicate gold data: date={new_date}, currency={new_currency}"
        )
        ...
        for item in existing_data:
            ...
            if existing_date == new_date and existing_currency == new_currency:
                logger.info(
                    f"Duplicate gold data found for date={new_date}, currency={new_currency}"
                )
                return file_path, file_created_date, file_last_modified_date

    elif api_type == 'btc':
        new_last_refreshed = data.get("Meta Data", {}).get(
            "6. Last Refreshed", ""
        )
        new_currency = data.get("Meta Data", {}).get("4. Market Code", "")
        logger.debug(
            f"Checking for duplicate BTC data: last_refreshed={new_last_refreshed}, currency={new_currency}"
        )

        for item in existing_data:
            existing_last_refreshed = item.get("Meta Data", {}).get(
                "6. Last Refreshed", ""
            )
            existing_currency = item.get("Meta Data", {}).get(
                "4. Market Code", ""
            )

            if (
                existing_last_refreshed == new_last_refreshed
                and existing_currency == new_currency
            ):
                logger.info(
                    f"Duplicate BTC data found for last_refreshed={new_last_refreshed}, currency={new_currency}"
                )
                return file_path, file_created_date, file_last_modified_date
